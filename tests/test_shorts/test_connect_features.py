...
"""Run ABCD assessment for CONNECT (C) features only"""
...
def run_evaluation():
    """Run ABCD Detector evaluation on test video with CONNECT features only"""
    
    print("\n" + "=" * 70)
    print("🎬 RUNNING ABCD ASSESSMENT WITH CONNECT (C) FEATURES ONLY")
    print("=" * 70 + "\n")
    ...
    # CONNECT features to evaluate (12 total)
    connect_features = [
        "c_people_overall",
        "c_people_early",
        "c_early_faces",
    ...
    config.set_parameters(
        ...
        run_shorts=True,
        features_to_evaluate=connect_features,  # Only CONNECT features
    ...
    print(f"🔗 Features: CONNECT (C) category only ({len(connect_features)} features)")
    ...
    try:
        ...
        print("\n" + "=" * 70)
        print("✅ CONNECT FEATURES EVALUATION COMPLETE!")
        print("=" * 70)
        print("\n🎉 Check output above for all CONNECT feature results")
        return 0
