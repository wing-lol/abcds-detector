...
"""Run ABCD assessment for DIRECT (D) features only"""
...
def run_evaluation():
    """Run ABCD Detector evaluation on test video with DIRECT features only"""
    
    print("\n" + "=" * 70)
    print("🎬 RUNNING ABCD ASSESSMENT WITH DIRECT (D) FEATURES ONLY")
    print("=" * 70 + "\n")
    ...
    # DIRECT features to evaluate (10 total)
    direct_features = [
    ...
    config.set_parameters(
        ...
        run_shorts=True,
        features_to_evaluate=direct_features,  # Only DIRECT features
    ...
    print(f"🎯 Features: DIRECT (D) category only ({len(direct_features)} features)")
    ...
    try:
        ...
        print("\n" + "=" * 70)
        print("✅ DIRECT FEATURES EVALUATION COMPLETE!")
        print("=" * 70)
        print("\n🎉 Check output above for all DIRECT feature results")
        return 0
